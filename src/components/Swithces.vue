<template>
      ...
      <div>
        ...
        <input v-model="swithce" type="text" class="border-2 m-2">
      </div>
      <div>
      ...
      <button class="bg-red-300 hover:bg-red-600 hover:shadow-xl flex flex-row text-center  p-2 m-2" @click="close()">kapat</button>
       ...
       <div class="m-4 justify-between">swithces <button class="rounded-full border-2 w-12 h-12" @click="swithcesAdd()">+</button></div>
      <div class="flex flex-col text-center" v-for="swithce in swithces" :key="swithce.id">
        <div class="border-2 p-4 " >{{swithce.text}}</div>
      </div>
      </div>
</template>
